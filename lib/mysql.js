...
Connection.prototype.connect = function(callback, errback) {
	errback = errback || this.defaultErrback;
	this.protocol = new Protocol(
			this.connect_parameter[0],
			this.connect_parameter[4]);
	if (this._timeout){
		this.protocol.timeout(this._timeout);
	}
	this.protocol.addListener('connect', utils.scope(this, function() {
		this.active = false;
		this.emit('connect');
		this.protocol.authenticate(this.connect_parameter[1],
				this.connect_parameter[2], this.connect_parameter[3],
	...
	this.protocol.addListener('error', errback);
};
...
Connection.prototype.autocommit = function(flag, callback, errback) {
	this.query("set autocommit=" + (flag ? "1" : "0"), callback,
			(errback || this.defaultErrback));
};

Connection.prototype.query = function(sql, callback, errback) {
	var _connection = this;
	var _protocol = this.protocol;
	var errback = errback || this.defaultErrback;
	...
	this.protocol.query_command(sql)
		.addCallback(utils.scope(this, function(nfields) {
			if (nfields) {
				_protocol.retr_fields(nfields)
					.addCallback(utils.scope(this, function(fields) {
						this.fields = fields;
						this.get_result()
							.addCallback(function(rows) {
								_connection.server_status = _protocol.server_status;
								try {
									if (callback){
										callback(rows);
									}
				...
				var result = {};
				result.affected_rows = _connection.affected_rows = _protocol.affected_rows;
				result.insert_id = _connection.insert_id = _protocol.insert_id;
				result.server_status = _connection.server_status = _protocol.server_status;
				result.warning_count = _connection.warning_count = _protocol.warning_count;
				result.info = _connection.info = _protocol.message;
				try {
					if (callback){
						callback(result);
					}
...
Connection.prototype.next_result = function(callback, errback) {
	if (!this.has_more_results()) {
		process.nextTick(new Error("ClientError", "No more results"));
		return;
	}
	...
	this.protocol.get_result()
		.addCallback(function(nfields) {
			_protocol.retr_fields(nfields)
				.addCallback(function(fields) {
					_connection.fields = fields;
					_connection.result_exist = true;
					_connection.get_result()
...
Stmt.prototype.prepare = function(query, callback, errback) {
	...
	this.protocol.stmt_prepare_command(query)
		.addCallback(utils.scope(this, function(statement_id, param_count, field_packets) {
				_stmt.statement_id = statement_id;
				_stmt.sqlstate = "00000";
				_stmt.param_count = param_count;
				_stmt.fields = field_packets.map(function(field_packet) {
...
Field.prototype.is_num = function() {
	return [ constants.field.TYPE_DECIMAL, constants.field.TYPE_TINY,
	         constants.field.TYPE_SHORT, constants.field.TYPE_LONG,
	         constants.field.TYPE_FLOAT, constants.field.TYPE_DOUBLE,
	         constants.field.TYPE_LONGLONG, constants.field.TYPE_INT24 ]
	         .indexOf(this.type.id) >= 0;
};
 ...
 * 
 * Author: masuidrive <masui@masuidrive.jp> License: MIT License Copyright (c)
 * Yuichiro MASUI
 *  # Original: # http://github.com/tmtm/ruby-mysql # Copyright (C) 2009-2010
 * TOMITA Masahiro # mailto:tommy@tmtm.org # License: Ruby's
 */
